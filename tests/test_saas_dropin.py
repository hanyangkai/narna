...
class BillingPlansTest(unittest.TestCase):
    def test_cloud_alias_and_caps(self) -> None:
        ...
        self.assertEqual(plan_adqa_soft_cap("free"), 5_000)
        self.assertIsNone(plan_adqa_hard_cap("free"))
...
class QuotaTest(unittest.TestCase):
    def test_free_soft_warn_no_402(self) -> None:
        """Free-first launch: ADQA has no hard cap — soft warn only."""
        ...
        org.adqa_checks_in_period = 5_001
        org.events_in_period = 0
        org.gu_in_period = 0
        org.period_start_at = datetime.now(timezone.utc)
        warn = enforce_plan_limit(org=org, projected_adqa=1)
        self.assertIsNotNone(warn)
        assert warn is not None
        self.assertTrue(warn.get("quotaWarning"))
        self.assertEqual(warn.get("metric"), "adqa")
